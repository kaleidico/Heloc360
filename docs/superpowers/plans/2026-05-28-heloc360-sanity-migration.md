...
This file is imported by both server-side code (`lib/sanity/api.ts`) AND client-side code (`components/blog/portable-text.tsx`). It MUST NOT import `./client.ts` because that file has `import 'server-only'` and would crash the client bundle. `@sanity/image-url`'s `imageUrlBuilder` accepts a `{ projectId, dataset }` config object as an alternative to a Sanity client — use that.
