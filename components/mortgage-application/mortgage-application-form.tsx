...
import { getMergedTrackingData, updateTrackingWithIp, initializeTracking } from '@/lib/tracking'
import { ArrowLeft, ArrowRight, Check, MapPin, User, FileText, Home, DollarSign, CreditCard, Shield, MessageSquare, UserCheck } from 'lucide-react'
...
export default function MortgageApplicationForm() {
  ...
  const getFieldsForStep = (step: number): (keyof FormData)[] => {
    switch (step) {
      ...
      case 9:
        return ['firstName', 'lastName', 'email', 'phone', 'contactPreference']
      default:
        return []
    }
  }
  ...
  const handleCurrencyChange = (field: 'propertyValue' | 'downPayment' | 'loanAmount') => (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <div className="text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-2">{stepTitles[currentStep - 1]}</h3>
          <p className="text-sm text-gray-500">Step {currentStep} of {totalSteps} • {Math.round((currentStep / totalSteps) * 100)}% Complete</p>
        </div>
      </div>
        ...
        {currentStep === 1 && (
          <div className="space-y-8">
            <div className="relative">
              ...
              <div className="relative">
                <p className="text-gray-600 text-lg">Let's start with finding your dream property location.</p>
              </div>
            </div>
            
            <div className="space-y-8">
              <div>
                <Label htmlFor="zipCode" className="text-base font-semibold text-gray-800 mb-3 block">Property Zip Code *</Label>
                {/* LastPass dummy field */}
                <input type="text" tabIndex={-1} autoComplete="off" readOnly className="hidden" />
                <Input
                  id="zipCode"
                  {...register('zipCode')}
                ...
                {locationData && (
                  <div className="mt-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl">
                    <p className="text-sm text-green-800 flex items-center font-medium">
                      ...
                      <strong>Location:</strong> {locationData.city}, {locationData.state}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div>
                <Label htmlFor="propertyAddress" className="text-base font-semibold text-gray-800 mb-3 block">Property Address</Label>
            ...
            {mapQuery && (
              <div className="mt-8">
                <Label className="text-base font-semibold text-gray-800 mb-3 block">Location Preview</Label>
        ...
        {currentStep === 2 && (
          <div className="space-y-8">
            <div className="relative">
              ...
              <div className="relative">
                <p className="text-gray-600 text-lg">Tell us a bit about yourself so we can find the best loan options for you.</p>
              </div>
            </div>
            
            <RadioGroup
              value={watchedValues.borrowerType}
              onValueChange={(value) => setValue('borrowerType', value as any)}
              className="space-y-4"
            >
              {[
                'Individual Homeowner (or Homebuyer)',
                   ...
                   >
                     <div className={`flex items-center justify-center w-6 h-6 border-2 rounded-full mr-4 transition-all duration-200 ${
                       ...
                       <div className={`w-3 h-3 bg-white rounded-full transition-all duration-200 ${
                         watchedValues.borrowerType === type ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
                       }`} />
                     </div>
                     <span className="text-base font-medium text-gray-800">{type}</span>
                   </Label>
                </div>
              ))}
            </RadioGroup>
        ...
        {currentStep === 3 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Tell us about your loan requirements.</p>
            </div>
            
            <div className="space-y-6">
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Mortgage Type</Label>
                <RadioGroup
                  value={watchedValues.mortgageType}
                  onValueChange={(value) => setValue('mortgageType', value as any)}
                  className="space-y-3"
                >
                  {['Purchase', 'Refinance', 'Cash Out'].map((type) => (
                    <div key={type} className="relative">
                      <RadioGroupItem value={type} id={type} className="sr-only" />
                      <Label 
                        htmlFor={type} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.mortgageType === type 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          watchedValues.mortgageType === type 
                            ? 'border-blue-500 bg-blue-500' 
                            : 'border-gray-300 group-hover:border-blue-400'
                        }`}>
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.mortgageType === type ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
              ...
              <div>
                <Label htmlFor="timeline" className="text-sm font-medium text-gray-700 mb-2 block">Timeline for Purchase</Label>
                <select
                  id="timeline"
                  {...register('timeline')}
                ...
                >
                  ...
                  <option value="Buy in 30 days">Buy in 30 days</option>
                  <option value="Buy in 60 days">Buy in 60 days</option>
                  <option value="Buy in 90 days">Buy in 90 days</option>
                  <option value="Buy in 6 months">Buy in 6 months</option>
                  <option value="Buy in 12 months">Buy in 12 months</option>
        ...
        {currentStep === 4 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Tell us about the property.</p>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  {[
                    ...
                    <div key={type} className="relative">
                      ...
                      <Label 
                        htmlFor={type} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.propertyType === type 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.propertyType === type ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
                        <span className="text-sm font-medium text-gray-700">{type}</span>
                      </Label>
              ...
              <div>
                ...
                <RadioGroup
                  ...
                  className="space-y-3"
                >
                  {['Yes (Owner Occupied)', 'No (Non-Owner Occupied)'].map((status) => (
                    <div key={status} className="relative">
                      <RadioGroupItem value={status} id={status} className="sr-only" />
                      <Label 
                        htmlFor={status} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.occupancy === status 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.occupancy === status ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Tell us about the financial details.</p>
            </div>
            
            <div className="space-y-6">
              <div>
                <Label htmlFor="propertyValue" className="text-sm font-medium text-gray-700 mb-2 block">Property Value *</Label>
                <Input
                  id="propertyValue"
                  ...
                  placeholder="$250,000"
                  autoComplete="off"
                  className={`h-12 ${errors.propertyValue ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'focus:border-blue-500 focus:ring-blue-500'}`}
                />
                {errors.propertyValue && (
                  <p className="text-sm text-red-500 mt-2">{errors.propertyValue.message}</p>
                )}
              </div>

              {watchedValues.mortgageType === 'Purchase' && (
                <div>
                  <Label htmlFor="downPayment" className="text-sm font-medium text-gray-700 mb-2 block">Down Payment</Label>
                  <Input
                    id="downPayment"
                    value={watchedValues.downPayment || ''}
                    onChange={handleCurrencyChange('downPayment')}
                    placeholder="$50,000"
                    autoComplete="off"
              ...
              {(watchedValues.mortgageType === 'Refinance' || watchedValues.mortgageType === 'Cash Out') && (
                <div>
                  <Label htmlFor="loanAmount" className="text-sm font-medium text-gray-700 mb-2 block">Loan Amount</Label>
                  <Input
                    id="loanAmount"
        ...
        {currentStep === 6 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Loan Position</h2>
              <p className="text-gray-600 max-w-md mx-auto">What position will this loan be in?</p>
            </div>
            
            <RadioGroup
              value={watchedValues.loanPosition}
              onValueChange={(value) => setValue('loanPosition', value as any)}
              className="space-y-3"
            >
              {[
                '1st (only mortgage)',
                ...
                <div key={position} className="relative">
                  <RadioGroupItem value={position} id={position} className="sr-only" />
                  <Label 
                    htmlFor={position} 
                    className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                      watchedValues.loanPosition === position 
                        ? 'border-blue-500 bg-blue-50' 
                        : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                    }`}
                  >
                    <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                      ...
                      <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                        watchedValues.loanPosition === position ? 'opacity-100' : 'opacity-0'
                      }`} />
                    </div>
        ...
        {currentStep === 7 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Credit & History</h2>
              <p className="text-gray-600 max-w-md mx-auto">Tell us about your credit and history.</p>
            </div>
            
            <div className="space-y-6">
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Estimated Credit Score</Label>
                <RadioGroup
                  value={watchedValues.creditScore}
                  onValueChange={(value) => setValue('creditScore', value as any)}
                  className="space-y-3"
                >
                  {[
                    'Excellent: 720+',
                    ...
                    <div key={score} className="relative">
                      <RadioGroupItem value={score} id={score} className="sr-only" />
                      <Label 
                        htmlFor={score} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.creditScore === score 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.creditScore === score ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Military Service (for VA loan eligibility)</Label>
                <RadioGroup
                  value={watchedValues.militaryService}
                  onValueChange={(value) => setValue('militaryService', value as any)}
                  className="space-y-3"
                >
                  {['Yes', 'No'].map((service) => (
                    <div key={service} className="relative">
                      ...
                      <Label 
                        htmlFor={service} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.militaryService === service 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.militaryService === service ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Bankruptcy/Foreclosure History</Label>
                <RadioGroup
                  value={watchedValues.bankruptcy}
                  onValueChange={(value) => setValue('bankruptcy', value as any)}
                  className="space-y-3"
                >
                  {['Yes', 'No'].map((history) => (
                    <div key={history} className="relative">
                      <RadioGroupItem value={history} id={`bankruptcy-${history}`} className="sr-only" />
                      <Label 
                        htmlFor={`bankruptcy-${history}`} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.bankruptcy === history 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.bankruptcy === history ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
        ...
        {currentStep === 8 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Any additional details you'd like to share?</p>
            </div>
            
            <div>
              <Label htmlFor="additionalInfo" className="text-sm font-medium text-gray-700 mb-2 block">Additional Information (Optional)</Label>
                                <Textarea
                id="additionalInfo"
        ...
        {currentStep === 9 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 max-w-md mx-auto">Finally, let's get your contact details.</p>
            </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Input
                    ...
                    className={`h-12 ${errors.firstName ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'focus:border-blue-500 focus:ring-blue-500'}`}
                  />
                  {errors.firstName && (
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={`h-12 ${errors.lastName ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'focus:border-blue-500 focus:ring-blue-500'}`}
                  />
                  {errors.lastName && (
              ...
              <div>
                ...
                <Input
                  ...
                  autoComplete="email"
                  className={`h-12 ${errors.email ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'focus:border-blue-500 focus:ring-blue-500'}`}
                />
                {errors.email && (
              ...
              <div>
                ...
                <Input
                  ...
                  autoComplete="tel"
                  className={`h-12 ${errors.phone ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'focus:border-blue-500 focus:ring-blue-500'}`}
                />
                {errors.phone && (
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Preferred Contact Method</Label>
                <RadioGroup
                  ...
                  {['Text', 'Phone', 'Email'].map((method) => (
                    <div key={method} className="relative">
                      ...
                      <Label 
                        htmlFor={method} 
                        className={`flex items-center p-4 border rounded-lg transition-all cursor-pointer group ${
                          watchedValues.contactPreference === method 
                            ? 'border-blue-500 bg-blue-50' 
                            : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                        }`}
                      >
                        <div className={`flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3 transition-colors ${
                          ...
                          <div className={`w-2.5 h-2.5 bg-white rounded-full transition-opacity ${
                            watchedValues.contactPreference === method ? 'opacity-100' : 'opacity-0'
                          }`} />
                        </div>
